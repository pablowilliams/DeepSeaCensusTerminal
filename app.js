...
const STARRED = [
  { ticker: "POLY", name: "Polychaeta — abyssal worms", sector: "Annelida", price: 920, mu: 0.05, sigma: 0.2 },
  { ticker: "ISOP", name: "Isopoda — deep-sea isopods", sector: "Crustacea", price: 720, mu: 0.06, sigma: 0.18 },
  { ticker: "FORAM", name: "Foraminifera — abyssal seabed", sector: "Protista", price: 1450, mu: 0.03, sigma: 0.12 },
  { ticker: "GASTRO", name: "Gastropoda — hydrothermal", sector: "Mollusca", price: 285, mu: 0.08, sigma: 0.28 },
  { ticker: "ECHIN", name: "Echinodermata — abyssal plain", sector: "Echinodermata", price: 410, mu: 0.04, sigma: 0.15 },
  { ticker: "CNIDA", name: "Cnidaria — cold-water corals", sector: "Cnidaria", price: 530, mu: 0.05, sigma: 0.16 },
  { ticker: "PORI", name: "Porifera — abyssal sponges", sector: "Porifera", price: 340, mu: 0.06, sigma: 0.22 },
  { ticker: "TUNI", name: "Tunicata — pelagic salps", sector: "Chordata", price: 175, mu: 0.04, sigma: 0.18 },
  { ticker: "CEPH", name: "Cephalopoda — abyssal octopods", sector: "Mollusca", price: 96, mu: 0.05, sigma: 0.2 },
  { ticker: "ELASMO", name: "Elasmobranchii — deep sharks", sector: "Chordata", price: 64, mu: 0.02, sigma: 0.14 },
  { ticker: "AMPHIPOD", name: "Amphipoda — hadal trenches", sector: "Crustacea", price: 380, mu: 0.09, sigma: 0.3 },
  { ticker: "MICROB", name: "Archaea — vent chemosynthetic", sector: "Archaea", price: 1180, mu: 0.12, sigma: 0.32 },
];
  ...
  "POLY": [
    {
      ...
      "author": "@worms_db",
      "handle": "1h",
      "text": "$POLY 47 new abyssal polychaete species described in CCZ this year. Description backlog still 1200+."
    },
    {
      "sent": "neu",
      "author": "@ifremer_blue",
      "handle": "1d",
      "text": "$POLY broad latitudinal range. CCZ mining sites overlap with several endemic populations."
    }
  ],
  "ISOP": [
    {
      ...
      "author": "@isopod_lab",
      "handle": "3h",
      "text": "$ISOP giant isopod Bathynomus jamesi formally described — Asia-Pacific abyssal. 18 cm length recovered."
    }
  ],
  "FORAM": [
    {
      ...
      "author": "@cosmo_chem",
      "handle": "5h",
      "text": "$FORAM benthic forams remain richest abyssal clade by described count. Phylogeny still in flux."
    }
  ],
  "GASTRO": [
    {
      ...
      "author": "@vent_biology",
      "handle": "2h",
      "text": "$GASTRO new scaly-foot gastropod variant discovered at Indian Ocean Kairei vent. Iron-sulfide armor confirmed."
    }
  ],
  "ECHIN": [
    {
      ...
      "author": "@echino_lab",
      "handle": "7h",
      "text": "$ECHIN abyssal echinoderms — holothurians dominate biomass on Pacific abyssal plain."
    }
  ],
  "CNIDA": [
    {
      ...
      "author": "@cold_corals",
      "handle": "4h",
      "text": "$CNIDA Lophelia pertusa reefs sequenced — clonal age 1000+ years confirmed on NE Atlantic mounds."
    }
  ],
  "PORI": [
    {
      ...
      "author": "@porifera_lab",
      "handle": "1d",
      "text": "$PORI abyssal hexactinellid sponge gardens — biomass hotspots in CCZ. Slow-growing, ~5 mm/yr."
    }
  ],
  "TUNI": [
    {
      ...
      "author": "@pelagic_team",
      "handle": "6h",
      "text": "$TUNI pyrosomes booming in mid-water layers. Climate-shift indicator."
    }
  ],
  "CEPH": [
    {
      "sent": "pos",
      "author": "@cephalopod_obs",
      "handle": "1h",
      "text": "$CEPH dumbo octopus (Grimpoteuthis) imaged at 6957 m off Mariana Trench. New depth record for the genus."
    }
  ],
  "ELASMO": [
    {
      "sent": "neu",
      "author": "@chondrichthyes",
      "handle": "11h",
      "text": "$ELASMO Greenland shark longevity reconfirmed — 270-400 yr range. Slow-growth abyssal cohort."
    }
  ],
  "AMPHIPOD": [
    {
      ...
      "author": "@hadal_zone",
      "handle": "2h",
      "text": "$AMPHIPOD Hirondellea gigas from Challenger Deep — full genome sequenced. Pressure-tolerance adaptations mapped."
    }
  ],
  "MICROB": [
    {
      ...
      "author": "@chemo_eco",
      "handle": "4h",
      "text": "$MICROB Lost City chimneys — Methanothermococcus consortium. Origin-of-life relevance growing."
    }
  ]
};
...
const STRATEGIES = [
  {
    id: "ma_cross",
    name: "Discovery MA Cross",
    desc: "20/50-yr rate",
    apply: (ctx) => {
      const s20 = sma(ctx.history, 20);
      const s50 = sma(ctx.history, 50);
      if (s20 == null || s50 == null) return { signal: "STABLE", detail: "Not enough history for 50-day MA." };
      if (s20 > s50 * 1.005) return { signal: "ENDEMIC",  detail: `20-day (${s20.toFixed(2)}) above 50-day (${s50.toFixed(2)}) — bullish trend.` };
      if (s20 < s50 * 0.995) return { signal: "INVASIVE", detail: `20-day (${s20.toFixed(2)}) below 50-day (${s50.toFixed(2)}) — bearish trend.` };
      return { signal: "STABLE", detail: `20-day ≈ 50-day — consolidation.` };
    },
  },
  {
    id: "rsi_mr",
    name: "Sampling MR",
    desc: "effort deviation",
    apply: (ctx) => {
      const r = rsi(ctx.history, 14);
      if (r < 30) return { signal: "ENDEMIC",  detail: `RSI ${r.toFixed(1)} — oversold, mean-revert up.` };
      if (r > 70) return { signal: "INVASIVE", detail: `RSI ${r.toFixed(1)} — overbought, mean-revert down.` };
      return { signal: "STABLE", detail: `RSI ${r.toFixed(1)} — mid-range, no edge.` };
    },
  },
  {
    id: "momentum",
    name: "Range Momentum",
    desc: "depth migration",
    apply: (ctx) => {
      const h = ctx.history;
      if (h.length < 21) return { signal: "STABLE", detail: "Not enough data." };
      const r20 = (h[h.length - 1] - h[h.length - 21]) / h[h.length - 21];
      if (r20 > 0.05)  return { signal: "ENDEMIC",  detail: `+${(r20 * 100).toFixed(1)}% past 20d — ride the trend.` };
      if (r20 < -0.05) return { signal: "INVASIVE", detail: `${(r20 * 100).toFixed(1)}% past 20d — negative momo.` };
      return { signal: "STABLE", detail: `${(r20 * 100).toFixed(1)}% past 20d — flat.` };
    },
  },
  {
    id: "mc_asymmetry",
    name: "Chao1 Skew",
    desc: "unobserved richness",
    apply: (ctx) => {
      if (!ctx.mcSummary) return { signal: "STABLE", detail: "Run a simulation." };
      ...
      if (ratio > 1.3 && expectedReturn > 0.01) return { signal: "ENDEMIC", detail: `Up/down ratio ${ratio.toFixed(2)}x, E[R] ${(expectedReturn * 100).toFixed(1)}%, Pr(up) ${(probUp * 100).toFixed(0)}%.` };
      if (ratio < 0.8 && expectedReturn < -0.01) return { signal: "INVASIVE", detail: `Up/down ratio ${ratio.toFixed(2)}x, E[R] ${(expectedReturn * 100).toFixed(1)}%, Pr(up) ${(probUp * 100).toFixed(0)}%.` };
      return { signal: "STABLE", detail: `Up/down ratio ${ratio.toFixed(2)}x — no clear edge.` };
    },
  },
  {
    id: "x_sentiment",
    name: "Biologist Notes",
    desc: "net field reports",
    apply: (ctx) => {
      const s = ctx.sentiment;
      if (s.score > 0.25)  return { signal: "ENDEMIC",  detail: `Net +${(s.score * 100).toFixed(0)} — crowd bullish.` };
      if (s.score < -0.15) return { signal: "INVASIVE", detail: `Net ${(s.score * 100).toFixed(0)} — crowd bearish.` };
      return { signal: "STABLE", detail: `Net ${(s.score * 100).toFixed(0)} — mixed crowd.` };
    },
  },
];

function signalScore(signal) { return signal === "ENDEMIC" ? 1 : signal === "INVASIVE" ? -1 : 0; }

function combineSignals(results) {
  if (!results.length) return { signal: "STABLE", detail: "Select at least one strategy." };
  const avg = results.reduce((a, r) => a + signalScore(r.signal), 0) / results.length;
  if (avg > 0.35)  return { signal: "ENDEMIC",    detail: "Majority bullish." };
  if (avg < -0.35) return { signal: "INVASIVE",   detail: "Majority bearish." };
  return { signal: "STABLE", detail: "Mixed strategy signals." };
}
...
function renderTickerSelect() {
  ...
  el.addEventListener("change", (e) => {
    ...
    renderExtra3Panel();
    renderExtra4Panel();
    announce(`Selected ${state.selectedTicker}. Clade detail loaded.`);
  });
}
...
function priceFmt(x) {
  if (x == null || !isFinite(x)) return "—";
  return Math.round(x).toLocaleString() + " spp.";
}
...
function signalBadge(signal, opts = {}) {
  const cls = signal === "ENDEMIC" ? "badge-buy" : signal === "INVASIVE" ? "badge-sell" : "badge-unsure";
  const icon = signal === "ENDEMIC" ? "▲" : signal === "INVASIVE" ? "▼" : "◆";
...
const kpiDefs = () => {
  ...
  return [
    { id: "kpi-value",  label: "Portfolio value", value: k.value,           fmt: (v) => dollarFmt(v), mod: "accent",                              sub: `${state.stocks.length} clades · equal weight` },
...
function renderKPIs() {
  ...
  // Right rail
  const buys = state.stocks.filter((x) => getCombinedSignalForStock(x).signal === "ENDEMIC").length;
  const sells = state.stocks.filter((x) => getCombinedSignalForStock(x).signal === "INVASIVE").length;
  ...
  setText("#rail-buys-sub", `${state.stocks.length} clades total`);
...
function renderStocksTable() {
  ...
  body.innerHTML = rows.map((s) => {
    ...
    const dotCls = s.combinedSignal.signal === "ENDEMIC" ? "buy" : s.combinedSignal.signal === "INVASIVE" ? "sell" : "unsure";
...
function renderDetail(onComplete) {
  ...
  $("#detail-subtitle").textContent = `Habitat: ${stock.sector} · Species count ${priceFmt(stock.price)} (${pctFmt(stock.change)}) · Drift μ=${(stock.mu * 100).toFixed(1)}%, vol σ=${(stock.sigma * 100).toFixed(1)}%.`;
...
function renderSentiment(stock) {
  ...
  const posts = s.posts.length ? s.posts : [{ sent: "neu", author: "@market_data", handle: "now", text: "No recent public chatter for this clade." }];
...
function renderSummary() {
  ...
  const buy = all.filter((x) => x === "ENDEMIC").length;
  const sell = all.filter((x) => x === "INVASIVE").length;
  const uns = all.filter((x) => x === "STABLE").length;
  const winner = buy > sell && buy > uns ? "ENDEMIC" : sell > buy && sell > uns ? "INVASIVE" : "STABLE";
  const agg = `${buy} ENDEMIC · ${uns} STABLE · ${sell} INVASIVE across ${all.length} clades.`;
...
function spawnParticles(origin, kind) {
  ...
  if (!layer) return;
  const count = kind === "ENDEMIC" ? 18 : 14;
  for (let i = 0; i < count; i++) {
    const p = document.createElement("div");
    p.className = "fx-particle" + (kind === "INVASIVE" ? " sell" : "");
    p.style.left = origin.x + "px";
    p.style.top = origin.y + "px";
    layer.appendChild(p);
    const angle = kind === "ENDEMIC"
      ? (Math.random() * Math.PI * 2)
      : (Math.PI / 2 + (Math.random() - 0.5) * 0.7);
    const speed = kind === "ENDEMIC" ? 40 + Math.random() * 80 : 20 + Math.random() * 40;
    const dx = Math.cos(angle) * speed;
    const dy = Math.sin(angle) * speed + (kind === "INVASIVE" ? 70 + Math.random() * 40 : 0);
...
function particlesForSignalFlip(row, signal) {
  if (signal !== "ENDEMIC" && signal !== "INVASIVE") return;
  const badge = row.querySelector(".badge");
  if (!badge) return;
...
function runBootSequence(onComplete) {
  ...
  const alreadySeen = sessionStorage.getItem("dsc.booted") === "1";
  ...
  const finish = () => {
    ...
    sessionStorage.setItem("dsc.booted", "1");
  ...
  const lines = [
    "> DSC-TERMINAL v4.7  (c) deepseacensusterminal",
    "> booting kernel ........................ <span class='ok'>OK</span>",
    "> mounting clade watchlist ........... <span class='ok'>OK</span>",
    `> loading ${STARRED.length} clades .................... <span class='ok'>OK</span>`,
    "> initializing monte carlo engine (GBM) .. <span class='ok'>OK</span>",
    "> hooking DSC sentiment stream ........ <span class='ok'>OK</span>",
...
// ========== Depth-pressure profile column ==========
function renderExtraPanel() {
  const container = document.getElementById("extra-content");
  if (!container || !state.stocks) return;
  const clade = state.stocks.find((s) => s.ticker === state.selectedTicker) || state.stocks[0];
  const w = 720, h = 360, padL = 80, padR = 12, padT = 18, padB = 22;
  const inW = w - padL - padR, inH = h - padT - padB;
  const minD = 0, maxD = 11000;
  const yScale = (d) => padT + (d / maxD) * inH;
  // Zone bands
  const zones = [
    { min: 0, max: 200, label: "EPIPELAGIC", color: "rgba(0,180,255,0.10)" },
    { min: 200, max: 1000, label: "MESOPELAGIC", color: "rgba(0,140,255,0.10)" },
    { min: 1000, max: 3000, label: "BATHYAL", color: "rgba(0,120,180,0.15)" },
    { min: 3000, max: 6000, label: "ABYSSAL", color: "rgba(0,80,140,0.20)" },
    { min: 6000, max: 11000, label: "HADAL", color: "rgba(0,40,100,0.32)" },
  ];
  // Per-clade depth range derived from sigma/mu/price
  const cladeRanges = {
    POLY: [1000, 6000], ISOP: [200, 8000], FORAM: [50, 6000], GASTRO: [800, 3500],
    ECHIN: [500, 6500], CNIDA: [200, 2500], PORI: [800, 5500], TUNI: [0, 1200],
    CEPH: [400, 7000], ELASMO: [200, 2500], AMPHIPOD: [3000, 11000], MICROB: [800, 7500],
  };
  const range = cladeRanges[clade.ticker] || [500, 5000];
  // Markers - synthesize 20 samples in clade's range
  const markers = [];
  for (let i = 0; i < 20; i++) {
    const r = (Math.sin(i * 1.7 + clade.price) * 0.5 + 0.5);
    markers.push(range[0] + r * (range[1] - range[0]));
  }
  const ariaLabel = `Depth profile 0 to 11000 meters. ${clade.ticker} ${clade.name} occurs from ${range[0]}m to ${range[1]}m. ${markers.length} sample occurrences shown.`;
  // Render
  const columnX = padL + 80;
  const columnW = 30;
  const bands = zones.map(z => `
    <rect x="${columnX}" y="${yScale(z.min)}" width="${columnW}" height="${yScale(z.max) - yScale(z.min)}" fill="${z.color}" stroke="#191c23" stroke-width="0.5"/>
    <text x="${columnX + columnW + 8}" y="${(yScale(z.min) + yScale(z.max)) / 2 + 3}" fill="#7f8693" font-size="9" font-family="JetBrains Mono, monospace">${z.label}</text>
    <text x="${padL - 6}" y="${yScale(z.min) + 11}" text-anchor="end" fill="#55596a" font-size="9" font-family="JetBrains Mono, monospace">${z.min}m</text>
  `).join("");
  const rangeRect = `<rect x="${columnX - 4}" y="${yScale(range[0])}" width="${columnW + 8}" height="${yScale(range[1]) - yScale(range[0])}" fill="#00cc99" fill-opacity="0.18" stroke="#00cc99" stroke-width="1.5"/>`;
  const pts = markers.map(d => {
    const y = yScale(d);
    const x = columnX - 12 + Math.sin(d * 0.01) * 8;
    return `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="3" fill="#5ce8c2" stroke="#000" stroke-width="0.5"/>`;
  }).join("");
  // Pressure axis
  const presLabels = [10, 200, 600, 1100].map(atm => {
    const d = atm * 10;
    return `<text x="${columnX + columnW + 110}" y="${yScale(d) + 3}" fill="#55596a" font-size="9" font-family="JetBrains Mono, monospace">${atm} atm</text>`;
  }).join("");
  container.innerHTML = `
    <svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${ariaLabel}">
      <text x="${padL - 6}" y="14" text-anchor="end" fill="#7f8693" font-size="9" font-family="JetBrains Mono, monospace">DEPTH</text>
      <text x="${columnX + columnW + 110}" y="14" fill="#7f8693" font-size="9" font-family="JetBrains Mono, monospace">PRESSURE</text>
      ${bands}${rangeRect}${pts}${presLabels}
      <text x="${columnX - 70}" y="${yScale(range[0]) - 4}" fill="#00cc99" font-size="10" font-family="JetBrains Mono, monospace">range min ${range[0]}m</text>
      <text x="${columnX - 70}" y="${yScale(range[1]) + 12}" fill="#00cc99" font-size="10" font-family="JetBrains Mono, monospace">range max ${range[1]}m</text>
    </svg>
  `;
  const tk = document.getElementById("extra-ticker"); if (tk) tk.textContent = clade.ticker;
  const sub = document.getElementById("extra-subtitle");
  if (sub) sub.textContent = `${clade.name} · depth range ${range[0]}–${range[1]} m · ${markers.length} samples synthesized · pressure ~${Math.round(range[1] / 10)} atm at max depth.`;
  const body = document.getElementById("extra-data-body");
  if (body) body.innerHTML = markers.map((d, i) => {
    const zone = zones.findLast ? zones.findLast(z => d >= z.min) : [...zones].reverse().find(z => d >= z.min);
    return `<tr><td>${Math.round(d)}</td><td>${zone?.label || ""}</td><td>${Math.round(d / 10)}</td><td>sample ${i + 1}</td></tr>`;
  }).join("");
}
...
function renderExtra2Panel() {
  ...
  if (!container || !state.stocks) return;
  const clades = state.stocks;
  const w = 720, h = 360, padL = 90, padR = 60, padT = 18, padB = 28;
  const inW = w - padL - padR, inH = h - padT - padB;
  const rowH = inH / clades.length;
  // Chao1 = S_obs + (f1^2 / (2 f2)), ACE = adjustment for abundance
  const data = clades.map(c => {
    const obs = Math.round(c.price);
    const f1 = Math.round(obs * (0.05 + c.sigma * 0.2));
    const f2 = Math.max(1, Math.round(f1 * 0.6));
    const chao = obs + Math.round((f1 * f1) / (2 * f2));
    const ace = obs + Math.round((f1 * f1) / (2 * f2)) + Math.round(obs * c.sigma * 0.5);
    return { ticker: c.ticker, obs, chao, ace };
  });
  const maxAce = Math.max(...data.map(d => d.ace));
  let bars = "";
  data.forEach((d, i) => {
    const y = padT + i * rowH;
    const wObs = (d.obs / maxAce) * inW;
    const wChao = (d.chao / maxAce) * inW;
    const wAce = (d.ace / maxAce) * inW;
    bars += `<text x="${padL - 6}" y="${y + rowH / 2 + 3}" text-anchor="end" fill="#d6dce4" font-size="10" font-family="JetBrains Mono, monospace">${d.ticker}</text>
      <rect x="${padL}" y="${y + 4}" width="${wAce.toFixed(1)}" height="${(rowH - 8).toFixed(1)}" fill="#1a2029" stroke="#008866" stroke-dasharray="3 2"/>
      <rect x="${padL}" y="${y + 4}" width="${wChao.toFixed(1)}" height="${(rowH - 8).toFixed(1)}" fill="#00cc99" fill-opacity="0.35"/>
      <rect x="${padL}" y="${y + 4}" width="${wObs.toFixed(1)}" height="${(rowH - 8).toFixed(1)}" fill="#00cc99"/>
      <text x="${padL + wAce + 8}" y="${y + rowH / 2 + 3}" fill="#5ce8c2" font-size="10" font-family="JetBrains Mono, monospace">ACE ${d.ace} · Chao1 ${d.chao} · obs ${d.obs}</text>`;
  });
  container.innerHTML = `<svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Chao1 and ACE non-parametric richness estimators per clade. Solid bar: observed. Dashed bar: ACE upper estimate.">
    <text x="${padL}" y="14" fill="#7f8693" font-size="9" font-family="JetBrains Mono, monospace">OBSERVED (solid) · CHAO1 (dim) · ACE (dashed outline)</text>
    ${bars}
  </svg>`;
  const body = document.getElementById("extra2-data-body");
  if (body) body.innerHTML = data.map(d => `<tr><td>${d.ticker}</td><td>${d.obs}</td><td>${d.chao}</td><td>${d.ace}</td></tr>`).join("");
}
...
function renderExtra3Panel() {
  ...
  if (!container) return;
  const data = [{"name":"f_2 doubletons","units":"+25%","down":-7.6,"up":5.4},{"name":"f_1 singletons","units":"+10%","down":-3.2,"up":3.8},{"name":"Sample coverage","units":"−5pp","down":-2.1,"up":2.4},{"name":"Cryptic-species rate","units":"+25%","down":-1.4,"up":4.6},{"name":"Effort heterogeneity","units":"+25%","down":-1.8,"up":1.6},{"name":"Depth-range width","units":"+25%","down":-0.6,"up":1.2}];
  ...
  const bars = data.map((d, i) => {
    ...
    const upBar = `<rect x="${xZero}" y="${y}" width="${ux.toFixed(1)}" height="${bh}" fill="#00cc99" fill-opacity="0.72"/>`;
    ...
    const upText = `<text x="${xZero + ux + 4}" y="${y + bh / 2 + 4}" fill="#00cc99" font-size="10" font-family="JetBrains Mono, monospace">+${d.up.toFixed(1)}%</text>`;
...
function renderExtra4Panel() {
  ...
  if (!container) return;
  const X = {"name":"f_1","values":[50,100,150,200,250,300]};
  const Y = {"name":"f_2","values":[25,50,75,100,150]};
  const label = "Chao1 − Observed (species)";
  const surface = (x, y) => Math.min(1, (x * x) / (Math.max(1, 2 * y) * 600));
  const palette = ["#1a2029", "#008866", "#00cc99", "#5ce8c2", "#ff3366"];
  const glyphs = [".", "·", "=", "▲", "■"];
